...
import type { Campaign, Member, Page, Prospect, Template } from '../types'

export function ProspectsPage() {
  const [page, setPage] = useState<Page<Prospect> | null>(null)
  const [search, setSearch] = useState('')
  const [modal, setModal] = useState<'create' | 'import' | null>(null)
  const [message, setMessage] = useState('')
  const load = useCallback(() => api<Page<Prospect>>(`/prospects?search=${encodeURIComponent(search)}&order=name`).then(setPage), [search])
  ...
  return <PageFrame title="Prospects" detail="Only add people from a source you are authorized to use. This product does not scrape or harvest platform data." actions={<><Button variant="secondary" onClick={() => { setMessage(''); setModal('import') }}><Upload size={17} />Import CSV</Button><Button onClick={() => { setMessage(''); setModal('create') }}><Plus size={17} />Add prospect</Button></>}>
    ...
    <Panel>{page?.items.length ? <div className="table-wrap"><table><thead><tr><th>Name</th><th>Organization</th><th>Provider</th><th>Consent context</th><th>Source</th></tr></thead><tbody>
      {page.items.map((prospect) => <tr key={prospect.id}><td><strong>{prospect.name}</strong><small>{prospect.contact_handle || 'No handle stored'}</small></td><td>{prospect.organization || '—'}</td><td>{prospect.provider}</td><td><Status value={prospect.consent_status} /></td><td><a className="table-action" href={prospect.source_url} target="_blank" rel="noreferrer">Open source <ArrowUpRight size={14} /></a></td></tr>)}
...
export function CampaignsPage({ member }: { member: Member; onMemberChange: (member: Member) => void }) {
  ...
  const [message, setMessage] = useState('')
  const load = useCallback(() => api<Page<Campaign>>('/campaigns?order=newest').then(setPage), [])
  useEffect(() => { void load() }, [load])
...
export function TemplatesPage() {
  const [page, setPage] = useState<Page<Template> | null>(null)
  const [modal, setModal] = useState(false)
  const [message, setMessage] = useState('')
  const load = useCallback(() => api<Page<Template>>('/templates?order=name').then(setPage), [])
  useEffect(() => { void load() }, [load])
  async function create(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setMessage('')
