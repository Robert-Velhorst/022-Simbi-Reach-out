...
export default function SettingsPage({ member, onMemberChange }: { member: Member; onMemberChange: (member: Member) => void }) {
  ...
  async function addMember(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setMessage(null)
    try { await post('/settings/team', Object.fromEntries(new FormData(event.currentTarget))); (event.currentTarget as HTMLFormElement).reset(); await load(); setMessage({ tone: 'success', text: 'Local team member added.' }) }
