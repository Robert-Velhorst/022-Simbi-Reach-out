...
export default function ReviewQueue({ member }: { member: Member; onMemberChange: (member: Member) => void }) {
  ...
  const selected = data.drafts.find((draft) => draft.id === selectedId) ?? data.drafts[0] ?? null
  const selectedKey = selected ? `${selected.id}:${selected.content_hash}` : ''
  const editor = edited?.key === selectedKey ? edited : { subject: selected?.subject ?? '', body: selected?.body ?? '' }
  ...
  return <div className="page review-page">
    ...
    {data.drafts.length ? <div className="review-layout">
      <Panel className="review-list" title={`${data.drafts.length} draft${data.drafts.length === 1 ? '' : 's'}`}>
        {data.drafts.map((draft) => <button key={draft.id} disabled={busy} className={`review-item ${draft.id === selectedId ? 'selected' : ''}`} onClick={() => { setSelectedId(draft.id); setChecked([]); setEdited(null); setMessage('') }}><div><strong>{draft.prospect_name}</strong><Status value={draft.state} /></div><span>{draft.campaign_name}</span><small>Quality {draft.quality_score}/100 · {formatDate(draft.updated_at)}</small></button>)}
      </Panel>
